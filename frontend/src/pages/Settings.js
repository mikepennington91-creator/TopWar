                                    ...
                                    <div className="space-y-2">
                                      ...
                                      <div className="space-y-2 rounded-sm border border-slate-800 bg-slate-950/40 p-3">
                                        {getAssignablePrimaryRoles(assignableRoles).map((roleOption) => (
                                          <label key={`${mod.username}-role-${roleOption}`} className="flex items-center gap-2 text-sm text-slate-300">
                                            <input
                                              type="checkbox"
                                              checked={(roleEdits[mod.username]?.role || mod.role || 'moderator') === roleOption}
                                              onChange={() => updateRoleEdit(mod.username, { role: roleOption })}
                                              disabled={loading}
